package com.au.mit.benchmark.network.protocols.tcp.threadsingle;
...
public class ServerSingleThread extends AbstractServer {
    private Logger logger = Logger.getLogger(ServerSingleThread.class.getName());
    private volatile boolean closing = false;
    private Thread mainThread = null;
    private ServerSocket serverSocket = null;

    public ServerSingleThread(ServerParams params) {
        super(params);
    }
    ...
    private static class ClientHandler extends AbstractClientHandler<Socket> {
        private final ServerSingleThread server;
        private final int X;

        ClientHandler(Socket socket, ServerSingleThread server, int X) {
            super(socket, server);
            this.server = server;
            this.X = X;
        }
